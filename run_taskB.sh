...
# task 
for i in $(seq 200 100 800) #inclusive
do
...
gnuplot -c $gnu_file2 "$path/throughput vs bttlnkRate.png" "Throughput VS Bottleneck Link Capacity" "Bottleneck Link Capacity" "$path/data_1.txt" 1 3 4 ${method}
gnuplot -c $gnu_file3 "$path/fairness index vs bttlnkRate.png" "Fairness Index VS Bottleneck Link Capacity" "Bottleneck Link Capacity" "Fairness Index" "$path/data_1.txt" 1 5 "newreno + ${method}"
...
# output file name | title | x label | file 1 | file1_1stcol | file1_2ndcol | file1_3rdcol | title 2
gnuplot -c $gnu_file2 "$path/throughput vs packet loss rate.png" "Throughput VS Packet Loss Rate" "Packet Loss Rate" "$path/data_2.txt" 2 3 4 ${method}
